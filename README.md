...
- **右键菜单**（按上下文显示）：有选区 → 「强制转换为公式」（把选中内容包装为公式：单行 → `$...$`，多行/跨 `<br>` → 局部行内公式保留换行、整段 → `$$...$$`，先复用修复引擎清破损）；选区覆盖公式节点 → 再加「还原为纯文本」；光标在公式内 → 只显示「还原为纯文本」；普通文字且无选择 → 不显示公式操作
- **跨块选择**：拒绝执行并提示逐块操作（跨块部分选择会改写整块内容、且可能改变 Heading/CodeBlock 语义，为数据安全撤回批处理）
