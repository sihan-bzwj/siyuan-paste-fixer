...
- **右键菜单**：「修复为公式」「还原为纯文本」两项（编辑器内右键即可；官方菜单事件优先，另有事件兜底与超时注入两级保障，菜单复用不会残留重复项）
- **顶栏 Σ 按钮**：点击弹出开关菜单——代码内容「修复公式」（智能↔强制修）、AI 数学/网页公式/混合内容「处理开关」（智能↔关闭自动处理）、场景提示开关、修复/还原命令；菜单项使用打开瞬间的选区快照（菜单点击时 selection 可能已失效）
- **设置面板**：设置 → 集市 → 已安装插件 → 「粘贴公式修复」→ 设置，四个场景可选 智能 / 始终修复 / 原样粘贴
- `Ctrl+P` 命令面板（同样使用最近编辑器内选区快照）
...
src/manual-action.ts  手动转换动作层（局部片段/整块保 ID/跨块拒绝/白名单保护）
