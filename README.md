...
- **右键菜单**（按上下文显示）：有选区 → 「强制转换为公式」（把选中内容包装为公式：单行 → `$...$`，多行/跨 `<br>` → 局部行内公式保留换行、整段 → `$$...$$`，先复用修复引擎清破损）；选区覆盖公式节点 → 再加「还原为纯文本」；光标在公式内 → 只显示「还原为纯文本」；**代码块/行内代码内、跨块选区不显示「强制转换」**；普通文字且无选择 → 不显示
- **跨块选择**：「强制转换为公式」拒绝（整块语义无法保证）；「还原为纯文本」允许——节点级操作，只还原完整选中的公式节点（inline-math 原地替换、公式块单独更新），普通段落/Heading/代码完全不碰
- **设置面板**：设置 → 集市 → 已安装插件 → 「粘贴公式修复」→ 设置，四个场景可选 智能 / 始终修复 / 原样粘贴
- **顶栏 Σ 按钮**：只负责自动处理开关——代码内容「修复公式」（智能↔强制修）、AI 数学/网页公式/混合内容「处理开关」（智能↔关闭自动处理）、场景提示开关
- `Ctrl+P` 命令面板：将选中内容强制转换为公式（使用最近编辑器内选区快照）
